(function() {
    ...
    // Accessing Firebase from global window.firebaseAuth
    const { auth, db, googleProvider, signInWithPopup, onAuthStateChanged } = window.firebaseAuth;
    ...
    function setupAuthListener(onUserAuthenticated, onUserLoggedOut) {
    ...
    async function getPlayerData(uid) {
        const playerRef = firebase.firestore().doc(`players/${uid}`);
        const snap = await playerRef.get();
        return snap.exists ? snap.data() : null;
    }

    async function createPlayerProfile(user) {
        const playerRef = firebase.firestore().doc(`players/${user.uid}`);
        await playerRef.set({
